...
class MayaGarment(wrappers.VisPattern):
    ...
    def _load_panel(self, panel_name, pattern_group=None):
        ...
        # Create geometry but dont use qw.qlCreatePattern -- it's not needed. I need to transform the curves to polygon
        panel_geom = qw.qlCreatePattern(curve_group)
        #panel_geom = cmds.polyExtrudeFacet(curve_group + '.f[*]', ltz=0.1)[0]
...
class Scene(object):
    ...
    def _init_arnold(self):
        ...
        cmds.setAttr('defaultArnoldRenderOptions.renderDevice', 0)      # turn off GPU rendering
