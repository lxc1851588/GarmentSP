...
# in this version the scale factor is always 1.
class VisPattern(core.ParametrizedPattern):
    ...
    def _draw_a_panel(self, drawing, panel_name):
        """
        Adds a requested panel to the svg drawing with given offset and scaling
        Assumes (!!) 
            that edges are correctly oriented to form a closed loop
    ...
    def _calculate_minx_miny_width_height(self):
        min_x = 0
        min_y = 0
        max_x = 0
        max_y = 0
        for panel_name in self.pattern['panels']:
            panel = self.pattern['panels'][panel_name]
            vertices = np.asarray(panel['vertices'])
            min_x = min(min_x, np.min(vertices[:, 0]))
            min_y = min(min_y, np.min(vertices[:, 1]))
            max_x = max(max_x, np.max(vertices[:, 0]))
            max_y = max(max_y, np.max(vertices[:, 1]))
        width = max_x - min_x
        height = max_y - min_y
        return min_x, min_y, width, height,max_x,max_y
    ...
    def _save_as_image(self, svg_filename, png_filename):
        ...
        min_x, min_y, width, height, max_x, max_y = self._calculate_minx_miny_width_height()
        # As we flip the y axis, we need to flip the height
        min_y = -max_y
        dwg = svgwrite.Drawing(svg_filename, size=(width, height), viewBox=f"{min_x} {min_y} {width} {height}")      
        #base_offset = [60, 60]
        #panel_offset_x = 0
...
if __name__ == "__main__":
    ...
    pattern = VisPattern("C:/Users/18515/Desktop/SP/pants_straight_sides_1000/pants_straight_sides_0A0YA5V0JM/specification.json")
